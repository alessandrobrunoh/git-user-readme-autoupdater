# Hi, Welcome to alessandrobrunoh's Profile! 👋

Full Stack Developer | Open Source Enthusiast | Tech Blogger
Building the future, one commit at a time.


## 🛠️ Technologies & Tools

**Languages:**  
`C++` • `Dart` • `Go` • `Java` • `JavaScript` • `Kotlin` • `Python` • `Ruby` • `Rust` • `TypeScript`

**Frameworks & Libraries:**  
`Actix Web` • `Angular` • `Express.js` • `Flutter` • `Next.js` • `React` • `Spring Boot` • `Svelte` • `Tokio`

**Databases & ORMs:**  
`Diesel` • `Hibernate` • `Sequelize`

**Tools & Build Systems:**  
`Gradle` • `Maven` • `Vite` • `Webpack`

**Testing:**  
`Cypress` • `JUnit` • `Jest` • `Vitest`

**Other:**  
`Actix` • `BLoC` • `Batchfile` • `C` • `CMake` • `CSS` • `Clap` • `DTrace` • `Dio` • `Dockerfile` • `Firebase` • `FreeMarker` • `HLSL` • `HTML` • `Handlebars` • `Hyper` • `Inno Setup` • `JHipster` • `Jackson` • `Jupyter Notebook` • `Just` • `Logback` • `MDX` • `Makefile` • `Metal` • `Nix` • `Objective-C` • `PLpgSQL` • `PowerShell` • `Procfile` • `Pug` • `Rayon` • `RenderScript` • `Requests` • `Reqwest` • `Rich Text Format` • `SCSS` • `Serde` • `Shell` • `Solidity` • `Swift` • `Tailwind CSS` • `Tree-sitter Query` • `Vue` • `WGSL` • `aiohttp`

---

## 📊 GitHub Statistics

### 📈 Overview

- 🗂️ **Total Repositories**: 44
- 📝 **Total Lines of Code**: 1,181,439
- ⭐ **Total Stars**: 19
- 🔱 **Total Forks**: 10

---

### 💻 Tech Stack / Languages

1. **Rust** - 78.8%
   ```
   ███████████████████████████████████████░░░░░░░░░░░
   ```

2. **Jupyter Notebook** - 11.6%
   ```
   █████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
   ```

3. **TypeScript** - 5.4%
   ```
   ██░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
   ```

4. **JavaScript** - 2.4%
   ```
   █░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
   ```

5. **Java** - 1.8%
   ```
   ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
   ```

---

### 🏆 Top 5 Repositories

1. **[LeptosTest](https://github.com/alessandrobrunoh/LeptosTest)** - ⭐ 1 | 🔱 0
   - 💻 Language: Rust | 📝 Lines: 2,766

2. **[SycamoreTest](https://github.com/alessandrobrunoh/SycamoreTest)** - ⭐ 1 | 🔱 0
   - 💻 Language: Rust | 📝 Lines: 1,245

3. **[KetchApp-Auth-Api](https://github.com/alessandrobrunoh/KetchApp-Auth-Api)** - ⭐ 1 | 🔱 2
   - 💻 Language: Rust | 📝 Lines: 930

4. **[DioxusTest](https://github.com/alessandrobrunoh/DioxusTest)** - ⭐ 1 | 🔱 0
   - 💻 Language: Rust | 📝 Lines: 401

5. **[Progetto-Big-Data](https://github.com/alessandrobrunoh/Progetto-Big-Data)** - ⭐ 1 | 🔱 1
   - 💻 Language: Jupyter Notebook | 📝 Lines: 79,128

---

### 📚 All Repositories

| Repository | Stars | Forks | Language | Lines |
|------------|-------|-------|----------|-------|
| [Progetto-Big-Data](https://github.com/alessandrobrunoh/Progetto-Big-Data) | ⭐ 1 | 🔱 1 | Jupyter Notebook | 79,128 |
| [Relazione-Sicurezza-Privacy](https://github.com/alessandrobrunoh/Relazione-Sicurezza-Privacy) | ⭐ 1 | 🔱 1 | TypeScript | 74,410 |
| [Progetto-Machine-Learning](https://github.com/alessandrobrunoh/Progetto-Machine-Learning) | ⭐ 1 | 🔱 1 | Jupyter Notebook | 31,308 |
| [KetchApp-Flutter](https://github.com/alessandrobrunoh/KetchApp-Flutter) | ⭐ 1 | 🔱 0 | Dart | 17,409 |
| [Progetto-Fondamenti-Web](https://github.com/alessandrobrunoh/Progetto-Fondamenti-Web) | ⭐ 1 | 🔱 2 | CSS | 8,259 |
| [Progetto-Ingegneria-Web](https://github.com/alessandrobrunoh/Progetto-Ingegneria-Web) | ⭐ 1 | 🔱 1 | Vue | 4,049 |
| [KetchApp-API](https://github.com/alessandrobrunoh/KetchApp-API) | ⭐ 1 | 🔱 0 | Java | 3,166 |
| [LeptosTest](https://github.com/alessandrobrunoh/LeptosTest) | ⭐ 1 | 🔱 0 | Rust | 2,766 |
| [Test-Runner-Game-Java](https://github.com/alessandrobrunoh/Test-Runner-Game-Java) | ⭐ 1 | 🔱 0 | None | 2,133 |
| [KetchApp-BFF](https://github.com/alessandrobrunoh/KetchApp-BFF) | ⭐ 1 | 🔱 0 | Java | 1,970 |
| [update-profile-stats-script](https://github.com/alessandrobrunoh/update-profile-stats-script) | ⭐ 1 | 🔱 0 | Python | 1,913 |
| [SycamoreTest](https://github.com/alessandrobrunoh/SycamoreTest) | ⭐ 1 | 🔱 0 | Rust | 1,245 |
| [KetchApp-Kafka](https://github.com/alessandrobrunoh/KetchApp-Kafka) | ⭐ 1 | 🔱 2 | Java | 934 |
| [KetchApp-Auth-Api](https://github.com/alessandrobrunoh/KetchApp-Auth-Api) | ⭐ 1 | 🔱 2 | Rust | 930 |
| [leptos_styles](https://github.com/alessandrobrunoh/leptos_styles) | ⭐ 1 | 🔱 0 | Makefile | 728 |
| [DioxusTest](https://github.com/alessandrobrunoh/DioxusTest) | ⭐ 1 | 🔱 0 | Rust | 401 |
| [ReactTest](https://github.com/alessandrobrunoh/ReactTest) | ⭐ 1 | 🔱 0 | TypeScript | 282 |
| [Tokio-TCP-Chat-Test](https://github.com/alessandrobrunoh/Tokio-TCP-Chat-Test) | ⭐ 1 | 🔱 0 | Rust | 119 |
| [gpuiTest](https://github.com/alessandrobrunoh/gpuiTest) | ⭐ 1 | 🔱 0 | Rust | 22 |
| [zed](https://github.com/alessandrobrunoh/zed) | ⭐ 0 | 🔱 0 | Rust | 829,826 |
| [Conclave-Review-Manager](https://github.com/alessandrobrunoh/Conclave-Review-Manager) | ⭐ 0 | 🔱 0 | Svelte | 51,828 |
| [SpringBootJhipsterTest](https://github.com/alessandrobrunoh/SpringBootJhipsterTest) | ⭐ 0 | 🔱 0 | Java | 21,976 |
| [Web-Site](https://github.com/alessandrobrunoh/Web-Site) | ⭐ 0 | 🔱 0 | None | 14,495 |
| [RustProject](https://github.com/alessandrobrunoh/RustProject) | ⭐ 0 | 🔱 0 | Rust | 9,651 |
| [AlbionManagerDiscord](https://github.com/alessandrobrunoh/AlbionManagerDiscord) | ⭐ 0 | 🔱 0 | Rust | 5,741 |
| [tree-sitter-jdl](https://github.com/alessandrobrunoh/tree-sitter-jdl) | ⭐ 0 | 🔱 0 | C | 3,954 |
| [Discord-Bot-Ruby](https://github.com/alessandrobrunoh/Discord-Bot-Ruby) | ⭐ 0 | 🔱 0 | None | 3,785 |
| [guildkeeper-rs](https://github.com/alessandrobrunoh/guildkeeper-rs) | ⭐ 0 | 🔱 0 | None | 2,696 |
| [tree-sitter-ftl](https://github.com/alessandrobrunoh/tree-sitter-ftl) | ⭐ 0 | 🔱 0 | C | 2,312 |
| [alessandrobrunoh.github.io](https://github.com/alessandrobrunoh/alessandrobrunoh.github.io) | ⭐ 0 | 🔱 0 | SCSS | 1,305 |
| [git-user-readme-autoupdater](https://github.com/alessandrobrunoh/git-user-readme-autoupdater) | ⭐ 0 | 🔱 0 | Python | 887 |
| [DevTaskTracker](https://github.com/alessandrobrunoh/DevTaskTracker) | ⭐ 0 | 🔱 0 | TypeScript | 663 |
| [extensions](https://github.com/alessandrobrunoh/extensions) | ⭐ 0 | 🔱 0 | JavaScript | 470 |
| [Rest-API](https://github.com/alessandrobrunoh/Rest-API) | ⭐ 0 | 🔱 0 | None | 328 |
| [Discord-Bot](https://github.com/alessandrobrunoh/Discord-Bot) | ⭐ 0 | 🔱 0 | None | 313 |
| [zed-grammar-jdl](https://github.com/alessandrobrunoh/zed-grammar-jdl) | ⭐ 0 | 🔱 0 | Tree-sitter Query | 36 |
| [GPUI-Multi-Page-Ai-Terminal](https://github.com/alessandrobrunoh/GPUI-Multi-Page-Ai-Terminal) | ⭐ 0 | 🔱 0 | None | 1 |
| [alessandrobrunoh](https://github.com/alessandrobrunoh/alessandrobrunoh) | ⭐ 0 | 🔱 0 | None | 0 |
| [Card-Game-Builder](https://github.com/alessandrobrunoh/Card-Game-Builder) | ⭐ 0 | 🔱 0 | None | 0 |
| [HomeOS-for-Families](https://github.com/alessandrobrunoh/HomeOS-for-Families) | ⭐ 0 | 🔱 0 | None | 0 |
| [Linear-Issues-Logs](https://github.com/alessandrobrunoh/Linear-Issues-Logs) | ⭐ 0 | 🔱 0 | None | 0 |
| [My-Zed-IDE-Snippets](https://github.com/alessandrobrunoh/My-Zed-IDE-Snippets) | ⭐ 0 | 🔱 0 | None | 0 |
| [Tasks-Snippets](https://github.com/alessandrobrunoh/Tasks-Snippets) | ⭐ 0 | 🔱 0 | None | 0 |
| [zed-grammar-fdl](https://github.com/alessandrobrunoh/zed-grammar-fdl) | ⭐ 0 | 🔱 0 | None | 0 |

---

🤖 This profile was automatically updated on 2025-10-22 09:56:44
